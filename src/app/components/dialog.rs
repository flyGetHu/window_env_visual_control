...
pub struct BatchDeleteConfirmDialog<'a> {
    pub show: &'a mut bool,
...
impl<'a> BatchDeleteConfirmDialog<'a> {
    pub fn new(
        show: &'a mut bool,
        user_count: usize,
        system_count: usize,
            ...
            .show(ui.ctx(), |ui| {
                ui.vertical(|ui| {
                    ...
                    if self.user_count > 0 {
                        ui.label(format!("用户变量: {} 个", self.user_count));
                    }
                    
                    if self.system_count > 0 {
                        ui.label(format!("系统变量: {} 个 (将被跳过)", self.system_count));
                    }
                    
                    ui.add_space(8.0);
                    ui.label("确定要删除选中的用户变量吗？");
